...
from collectors import fetch_mootdx_5m, fetch_eastmoney_5m, fetch_sina_5m, fetch_yahoo_recent_5m, fetch_yahoo_daily
...
def safe(call, label):
    try: return call()
        ...
        print(label, "FAILED", repr(e)); return pd.DataFrame()
...
def main():
    ...
    for symbol, cfg in A_SHARES.items():
        ...
        for x in (td, sn, em):
            if not x.empty: x.drop(x[(x.ts < pd.Timestamp(START_DATE)) | (x.ts >= pd.Timestamp(END_DATE)+pd.Timedelta(days=1))].index, inplace=True)
    ...
    # Long daily macro history is used only for expanding/trailing state estimates available at each replay time.
    macro_start = (pd.Timestamp(START_DATE)-pd.DateOffset(years=3)).strftime("%Y-%m-%d")
    for ticker, label in [("^TYX","tyx"),("CL=F","oil"),("^SOX","sox")]:
        d=safe(lambda t=ticker: fetch_yahoo_daily(t,macro_start,END_DATE),f"daily macro {ticker}")
