...
def fetch_mootdx_5m(symbol,start,end,is_index=False,pages=5):
    ...
    old=socket.getdefaulttimeout(); socket.setdefaulttimeout(6); client=None
    try:
        client=Quotes.factory(market="std",multithread=False,heartbeat=False,bestip=False,timeout=6)
        parts=[]
        for page in range(pages):
            pos=page*800
            raw=client.index(symbol=code,frequency=0,market=market,start=pos,offset=800) if is_index else client.bars(symbol=code,frequency=0,start=pos,offset=800)
            part=_tdx_frame(raw)
            if part.empty: break
            parts.append(part)
...
def _fetch_yahoo(ticker,start,end,interval):
    ...
    return pd.DataFrame(rows)
def fetch_yahoo_5m(ticker,start,end): return _fetch_yahoo(ticker,start,end,"5m")
def fetch_yahoo_daily(ticker,start,end): return _fetch_yahoo(ticker,start,end,"1d")
def fetch_yahoo_recent_5m(ticker,start,end):
    now=pd.Timestamp.now(tz="UTC").tz_localize(None).normalize(); s,e=pd.Timestamp(start),pd.Timestamp(end); s=max(s,now-pd.Timedelta(days=59))
