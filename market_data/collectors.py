...
def _tdx_frame(df, source="mootdx"):
    ...
    out = pd.DataFrame({"ts": pd.to_datetime(x[dt_col], errors="coerce"),
        "open": pd.to_numeric(x["open"], errors="coerce"), "high": pd.to_numeric(x["high"], errors="coerce"),
        "low": pd.to_numeric(x["low"], errors="coerce"), "close": pd.to_numeric(x["close"], errors="coerce"),
        "volume": pd.to_numeric(x[vol_col], errors="coerce") if vol_col else 0,
        "amount": pd.to_numeric(x[amt_col], errors="coerce") if amt_col else None, "source": source})
...
def fetch_mootdx_5m(symbol: str, start: str, end: str, is_index: bool = False, pages: int = 5) -> pd.DataFrame:
    ...
    old_timeout = socket.getdefaulttimeout(); socket.setdefaulttimeout(8); client = None
    try:
        client = Quotes.factory(market="std", multithread=False, heartbeat=False, bestip=True, timeout=8)
        parts = []
        for page in range(pages):
            start_pos = page * 800
            raw = client.index(symbol=code, frequency=0, market=market, start=start_pos, offset=800) if is_index else client.bars(symbol=code, frequency=0, start=start_pos, offset=800)
            part = _tdx_frame(raw)
            if part.empty: break
            parts.append(part)
            if part["ts"].min() <= pd.Timestamp(start): break
...
def fetch_baostock_5m(symbol: str, start: str, end: str) -> pd.DataFrame:
    ...
    market = "sh" if symbol.startswith("sh") else "sz"; code = symbol[-6:]; bs_code = f"{market}.{code}"
    ...
    try:
        ...
        try:
            rs = bs.query_history_k_data_plus(bs_code,"date,time,code,open,high,low,close,volume,amount,adjustflag",start_date=start,end_date=end,frequency="5",adjustflag="3")
            if rs.error_code != "0": raise RuntimeError(f"baostock query {bs_code} {rs.error_code}: {rs.error_msg}")
            rows=[]
            while rs.next():
                p=dict(zip(rs.fields,rs.get_row_data())); ts=pd.to_datetime((p.get("time") or "")[:14],format="%Y%m%d%H%M%S",errors="coerce")
                if pd.isna(ts): continue
                rows.append({"ts":ts,"open":float(p["open"]),"high":float(p["high"]),"low":float(p["low"]),"close":float(p["close"]),"volume":float(p["volume"] or 0),"amount":float(p["amount"] or 0),"source":"baostock"})
...
def fetch_eastmoney_5m(secid: str, start: str, end: str) -> pd.DataFrame:
    ...
    r=requests.get(url,params=params,headers=HEADERS,timeout=10); r.raise_for_status(); klines=(((r.json().get("data") or {}).get("klines")) or []); rows=[]
    for v in klines:
        p=v.split(",")
...
def fetch_sina_5m(symbol: str, count: int = 1023) -> pd.DataFrame:
    url="https://money.finance.sina.com.cn/quotes_service/api/json_v2.php/CN_MarketData.getKLineData"; params={"symbol":symbol,"scale":5,"ma":"no","datalen":min(count,1023)}
    r=requests.get(url,params=params,headers=HEADERS,timeout=10); r.raise_for_status(); arr=r.json(); rows=[]
...
def _fetch_yahoo(ticker: str, start: str, end: str, interval: str) -> pd.DataFrame:
    ...
    url=f"https://query1.finance.yahoo.com/v8/finance/chart/{ticker}"; params={"period1":p1,"period2":p2,"interval":interval,"includePrePost":"false","events":"div,splits"}
    r=requests.get(url,params=params,headers=HEADERS,timeout=10); r.raise_for_status(); result=((r.json().get("chart") or {}).get("result") or [])
    if not result: return pd.DataFrame()
    res=result[0]; ts=res.get("timestamp") or []; q=(((res.get("indicators") or {}).get("quote") or [{}])[0]); rows=[]
    for i,u in enumerate(ts):
        vals={k:(q.get(k) or [None]*len(ts))[i] for k in ["open","high","low","close","volume"]}
        if vals["close"] is not None: rows.append({"ts":pd.to_datetime(u,unit="s",utc=True),"open":vals["open"],"high":vals["high"],"low":vals["low"],"close":vals["close"],"volume":vals["volume"],"amount":None,"source":"yahoo"})
    return pd.DataFrame(rows)


def fetch_yahoo_5m(ticker: str, start: str, end: str) -> pd.DataFrame: return _fetch_yahoo(ticker,start,end,"5m")
def fetch_yahoo_daily(ticker: str, start: str, end: str) -> pd.DataFrame: return _fetch_yahoo(ticker,start,end,"1d")


def fetch_yahoo_recent_5m(ticker: str, start: str, end: str) -> pd.DataFrame:
    now_utc=pd.Timestamp.now(tz="UTC").tz_localize(None).normalize(); s,e=pd.Timestamp(start),pd.Timestamp(end); s=max(s,now_utc-pd.Timedelta(days=59))
